      ...
      implements ActionListener, Observer
{
    ...
    protected JPanel createButtonPanel()
    {
		...
		(Tasks.class.getResource("icon.bmp"));
        
        if(icon.getIconHeight() <16)
        	System.out.println("Opaopaopaopapa");
    ...
    public void actionPerformed(ActionEvent evt)
    {
        ...
        try {
            ...
            case '+':
                model.setOperation(TasksModel.OP_ADD);
                break;
            case '-':
                model.setOperation(TasksModel.OP_SUB);
                break;
            case '*':
                model.setOperation(TasksModel.OP_MUL);
                break;
            case '/':
                model.setOperation(TasksModel.OP_DIV);
                break;
            case '=':
                model.calculate();
                break;
